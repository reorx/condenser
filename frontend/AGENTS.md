...
| `XVerdictDetail` | The pane's 判定 row: the verdict, its score, the labeled neighbours that voted (author handle + distance, linking to the original) and the `model@dims / algo` version. This trail is why Phase 4 badges instead of hiding — a verdict you can audit is one you can learn to trust, or catch being wrong and correct with a thumb |
