...
| `ItemDetailInfo` | The pane's top full-info label/value list, source-dispatched: TG = channel (avatar/name/@username), author, publish/edit times, forward origin, media count, item key; HN = source/type, author, submitted / front-page (上榜) / **admission (入选)** times — the gap between the last two is what the story spent earning its slot, and the pane is the only place a reader can see it; a story that was never admitted (search reaches those) has no 入选时间 row at all — score/comments (the comments link carries `hnLinkAttrs`, like the pane's footer link), 当日入选第 N 条 + peak rank, domain, item key (no AI 摘要 row on HN or RSS since 2026-09-19 — `ItemDetailBody` draws the block, and the row made the same words appear twice in one drawer); X = author (avatar/name/@handle → profile), which feed it came from, publish + probe-fetch times, engagement, RT/quote/reply origin, media count, your 反馈 label when set (with its reason chip — the card shows the reason nowhere, so this is where you check what a past thumbs-down actually meant), verdict (Phase 4), item key |
...
| `ItemDetailBody` | The pane's body section — the item's own text rendered **annotatable** (wrapped in `AnnotatedText`), the web counterpart of iOS's four detail sheets growing highlights. Per source, mirroring iOS's choices: TG = the message text (linkified), HN = the sanitized self-post HTML (an external-link story has none) under the story's `AiSummaryBlock` when it has a summary — iOS `HnDetailSheet`'s meta → summary → body, and the summary alone is enough to render the section, X = the derived display text (`xBodyText` — the same derivation `XCard` prints, so quotes relocate across surfaces and platforms), RSS = the full article, fetched lazily (`useRssArticle`; a saved snapshot's inline `content` skips the fetch) with the `AiSummaryBlock` above it (outside `AnnotatedText`, deliberately **not** annotatable — machine words; iOS's rule; the same goes for HN's). While the article is not in hand the excerpt keeps the section honest and highlighting stays off (a quote made against the excerpt would relocate against different text); a failed fetch keeps the excerpt + 「正文加载失败」. **An X long-form post** (`article.title` set) takes RSS's shape instead of `xBodyText` (plan 2026-09-16): its title as a heading outside the highlightable body, then `content_html` fetched via `useXArticle` (`GET /api/x/tweets/{id}`; a snapshot's inline copy skips it) and rendered `proxyImages(sanitizeHtml(...))` — images through `/api/preview/image`, like every tweet image. Two differences from RSS, both because the body can land *after* the list did (the probe reads it with the tweet, and a failed read is retried next round): it is fetched even when the list said `has_content: false` (and an answer without a body is not cached forever — `staleTime` is a function), but only a promised body earns 「正在加载全文…」 / 「正文加载失败」. Once the server has answered without a body, the preview stands with 「未获取到 article 正文」 under it — also when the request for an unpromised body fails, since the list already said it was missing; 「正文加载失败」 stays reserved for a promised body the request could not bring. `ARTICLE_PROSE` grew heading / list / blockquote / figure + figcaption styles for it (preflight strips them — RSS bodies benefit too) and `object-contain` on images, which now arrive sized. Renders nothing only when there is no body, no summary *and* nothing was ever highlighted — with annotations but no body the layer still mounts so the orphan list can say so |
...
| `AiSummaryBlock` + `displaySummary` | The AI summary as a quote block, the web twin of iOS `AiSummaryBlock` (2026-09-19): faint indigo ground, a straight 3px bar of the same hue **clipped** by the 8px corners (a `::before`, not a `border-l` — that one bends around the radius and tapers into a bracket), and the `Sparkles` 「AI 摘要」 label at the **top, inside** the block — "a machine wrote this" is met before the words, and label + words share one box. Every surface that shows a summary uses it: `HnCard`, `RssCard`, and `ItemDetailBody` for both sources. Takes `summary` + a margin-only `className`; renders nothing when the trimmed summary is empty (`displaySummary`, Kit's rule of the same name — exported for the callers that *branch* on having one: RssCard's excerpt fallback, HnCard's preview-description drop). No line clamp, unlike iOS's card: the column is wide enough and the summary is why the card exists |
...
| `RssSection` | The RSS tab on the Subscriptions page: add a feed by URL, bulk-import an OPML export (the file is read **in the browser** and posted as text, so an import goes through the same path a manual add does and cannot create a subscription typing one could not), the `RssSubscriptionRow` list, and a polling status line. Its `refetchInterval` is a **function**, not a constant (`rssRefetchInterval`, exported for its test): 5s while a feed still has no verdict, 60s once every one of them does. Right after an import that is the whole list and the first round lands within seconds — without it the rows sit on "waiting for the first fetch" until the reader reloads, which reads as a feed that never ran. "No verdict" is all three of `enabled && !fetched_at && error_count === 0`, not `fetched_at` alone: a failed round leaves `fetched_at` NULL on purpose, so a broken feed (or one paused before its first round) used to hold the page at 5s for as long as it stayed open. Rows are ordered by `sortRssSubscriptions` in three tiers — **abnormal** (server-marked: past the failure streak, backed off toward weekly), then merely failing (`error_count > 0`), then everything else in the server's `added_at desc` order, stably — nothing is auto-unsubscribed or paused, so surfacing the broken ones is still the mechanism, and an unstable order would reshuffle 77 rows under the reader's cursor on every refetch. The per-row **Refresh** mutation (`api.rssRefresh`, 2026-09-16) says its outcome in words — success + how many new, or 仍然失败 + the error verbatim — because the click was a question. The status line adds `N 个异常 feed 已退避` and the round's `deferred` count (requests the backoff saved). The import toast always states all three counts, since "added 40" alone hides that 12 were unusable. The status block is **two lines**: polling state, then the summary pipeline (`AI 摘要 <model> · 已生成 N · 待处理 N`, or the "no key configured" sentence with the count it *would* work through) — the only place a reader can tell a server with no summary key apart from entries too short to need one |
| `RssSubscriptionRow` | One feed row, Miniflux's shape since 2026-09-16: name (the URL until a fetch teaches us the title), a facts line (`rssFeedStatusParts`, exported for its test: `last check …` from `checked_at`, `last seen …` from `fetched_at` when failing — never-worked vs worked-then-died — and `next check …` only when the feed has a backoff time of its own), the error line `N errors – <last_error verbatim>`, a Refresh button, pause switch, unsubscribe. Three states are kept apart: **abnormal** (server-decided `abnormal`: yellow row + 「异常」 badge — the one waiting on the reader), `error_count > 0` broken and retrying on its own (red triangle), and a `last_error` with a zero count = a malformed-XML warning we recovered entries from anyway (amber triangle) |
