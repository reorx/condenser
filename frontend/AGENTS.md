...
| `RssCard` | One feed entry: the feed name as the header subject (falling back to the URL, scheme stripped — with 100 feeds a row is told apart by host), the title as the main act linking to the article, and a body that is the **LLM summary when there is one** and the article's plain-text excerpt when there is not (plan §0.4). Which of the two you are reading is marked with a small 「AI 摘要」 chip, because a summary is a machine's paraphrase and a card that hides that is lying quietly. Since 2026-08-23 the list payload carries **no HTML at all** — a ~500-char `content_excerpt` instead (feed bodies averaged 13.9KB and topped out at 7.1MB, thirty per page) — so the collapsed card renders prose the backend already stripped, and **"more" fetches the article** (`useRssArticle` → `GET /api/rss/entries/{id}`, `staleTime: Infinity`; a published document does not change under us) before rendering it sanitized. Three rules: the toggle appears on the server's `content_truncated`, not on a length guess here (only the cutting side knows what was left behind); a failed fetch **keeps the excerpt** + 「正文加载失败」 rather than blanking the body; and embedded images in the fetched article are **height-capped** (`max-h-80`) — one full-resolution photo makes a card taller than the viewport, at which point the timeline stops being scannable. Time opens the `ItemDetailPane`; its tooltip names the sort position too whenever the feed's declared time was not believed. An entry with no link renders a plain title |
