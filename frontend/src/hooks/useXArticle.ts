 ...
 * is there from the moment the entry is, but an X article's arrives later — the
 * probe fetches it at the end of a round — so an answer *without* a body is only
 * true for now and must not be cached forever. An answer with one is final (a
 * published tweet does not change under us). Pass null to keep it idle.
 */
