...
7. **生产跑的是旧代码**（2026-07-28 发现，**仍未解决**）：生产库里没有 `x_attributes` 表，
   schema 停在 v9，即步骤 0–5b 一行都没上线。因为默认 `channels=b`、行为逐字节不变
   （有测试钉住，且 §10.6 用真实数据复验过 100 条判定零变化），部署本身是安全的；
   但在部署之前，C/D 在生产上不会留下任何分数，前瞻监控对它们永远无话可说。
   **这是当前唯一的阻塞项**：部署 + 设 `CONDENSER_VERDICT_SHADOW_CHANNELS=c,d`
   + `CONDENSER_ATTR_API_KEY` → 攒 1~2 周 → 步骤 5c。
