...
| 5 | 按修订版 §9 走「准入 + badge-only 前瞻验证」：够门槛的通道负向即可开（首个候选 D）；`topic` 踩攒到 ~30 后回测 B 的 `topic_only_negatives` 开关 | 4 | 逐通道准入 |
...
0. **下一步是一次准入决策，不是代码任务**（步骤 5）。步骤 4 之后，能过 §9 数字门槛的操作点
   已经有了（b,d 投票 93.8%/16，D 单独 86.7%/15），要决定的是**开哪一个**。开之前还缺一个
   小工件：**前瞻精确度统计脚本**——对「先有判定、后有标注」的推算线上负精确度
   （`x_feed_items.verdict` 与 `item_feedback.created_at` 的时序比对，数据都在库里，
   不需要新表）。这是修订版 §9 的监控半边，没有它就只有准入没有复核。
1. **标注量仍是主要约束，但不再是「全有或全无」**（2026-07-28 修订）。系统本来就是渐进生效的：
   per-flag 的 `min_observations` 门槛 + 训练集实时重算，意味着某个 flag 的标注一过线就自动
   开始投票，无需改代码或配置。所以「等所有 flag 攒够」是错误的读法——`promo_cta` 今天就够用，
   其余 flag 到量那天自己加入。中间期该做的仍是继续标 + 提高 chips 覆盖率。
