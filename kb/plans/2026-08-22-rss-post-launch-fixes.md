...
## 3. 【已决策 → 2026-09-16 推翻】坏源不加退避 —— 由读者确认后手动关闭开关

> **2026-09-16 更新**：本节末尾留的退路兑现了——三周后生产 11/77 个坏源仍每轮失败、一个也没被
> 手动关掉，「注意到 → 关掉」的回路被证明太长。现已改为指数退避 + 「异常」标记 + 手动 Refresh，
> 见 `2026-09-16-rss-failure-backoff.md`。「不自动退订、不自动暂停」保留。
...
- 自动退订、自动暂停任何源（§3 已决策：判断谁是死源是读者的事）。~~自动退避~~ 2026-09-16 已加。
