...
状态：**已完成**（2026-08-23，commit `bb8e6a7` 后端 / `a39dab6` web / `3bdc361` iOS）
落地记录与量化见 `kb/docs/status-and-gaps.md` 同日条目；实施时的取舍（快照存全文、
回填 marker 记版本号、`condenser/text.py` 的由来）写在那里和各模块注释里。
