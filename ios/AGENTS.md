...
- **正文是 `RssBody` 而不是一个字符串**：`.summary` / `.excerpt` 两个 case。摘要是
  机器的转述，卡片必须标出来，只传字符串就没法区分。标法是 `AiSummaryBlock`
  （2026-08-23，卡片与详情 sheet 共用）：浅靛蓝底 + 左侧深一档的同色竖条的引用块，
  「AI 摘要」标注在块内顶部——眼睛先撞上标注再读内容，靛蓝是因为琥珀/橙/天蓝都已
  有含义。详情 sheet 两样都给——摘要块在上、全文接在下面，因为点进来是要读文章的。
