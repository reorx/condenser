...
/// 文章全文的一个渲染块：详情界面把文字交给文本视图、图片交给图片视图，
/// 图片才能占到自己的位置（纯文本管线里 `<img>` 没有内容，整个消失）。
///
/// 与信源无关：RSS 的 `content` 与 X 长文的 `content_html`（服务端从 Markdown
/// 渲染、图片带 width/height、图注走 `<figcaption>`）走的是同一条管线——
/// 2026-09-16 从 `RssBlocks` 改名而来，行为未变。
...
/// 顺序对读者是对的，少一个概念。
public struct ArticleImage: Equatable, Sendable {
    /// 已解析成绝对 URL（相对路径按文章 link 解析；X 长文的 URL 本来就是绝对的，
    /// 传 `baseURL: nil`），只会是 http/https——
...
public func articleBlocks(fromHTML html: String, baseURL: URL?) -> [ArticleBlock] {
...
/// `<img>` 换成带下标的占位符，属性解析成 `ArticleImage`。解析不出 src 的
/// （缺失且无 lazy-load 兜底、或全是 data URI）直接删掉——与纯文本管线里
/// 「img 整个消失」同一个结果，也因此不会占用下标。
private func liftImages(_ html: String, baseURL: URL?) -> (text: String, images: [ArticleImage]) {
...
/// 一个 `<img>` 标签 → ArticleImage。src 缺失或是 data: 占位图时回落
/// `data-src` / `data-original`——很多 WordPress lazy-load 插件就是这么发的。
private func parseImage(_ tag: String, baseURL: URL?) -> ArticleImage? {
    var raw: String?
...
private func split(_ text: String, images: [ArticleImage]) -> [ArticleBlock] {
