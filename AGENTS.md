...
| `channels.py` | the vocabulary the verdict's channels share (v2 plan): `ChannelScore` (score in [−1,+1], `confidence` = *how much evidence*, `corroborated` = may this carry a negative verdict, `meta`) and `combine`. **Abstaining is `None`, never 0.0** — folding silence in as a zero vote lets a channel that never fires drag the ones that do toward neutral. Nothing in the running verdict combines anything yet (that is step 4); today it is the type `verdict.topic_score` and `ngram.score` both speak, so the backtest can put them on one scale |
