...
⚠️ **Production still runs pre-step-0 code** (no `x_attributes`, schema v9), so C and D have never
scored a production tweet and the prospective monitor has nothing to say about them. Deploying is
safe by construction (`channels=b` is byte-identical, pinned by a test and re-verified above); it
needs `CONDENSER_VERDICT_SHADOW_CHANNELS=c,d` and `CONDENSER_ATTR_API_KEY` (~$0.01/day) to start
