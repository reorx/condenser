...
**Phase 3 is the real fix and should follow soon**: `qualified_at` + a per-hour admission
budget, which also closes a defect that happens *every* day today — a story becomes visible
hours after the position it gets sorted into, so it lands above the reader's cursor and
`/timeline/new` never reports it (plan §5.1).
