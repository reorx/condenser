...
| `items.py` | item keys (`tg:{cid}:{mid}` / `hn:{sid}` ↔ `(source, ref1, ref2)` triple) + the item **envelope** (`{source, key, datetime, is_read, is_saved, telegram\|hn}`) shared by timeline + records; the hn payload carries `preview` (`_parse_preview` accepts the stored JSON str, an already-parsed dict from saved-record replay, or None) |
